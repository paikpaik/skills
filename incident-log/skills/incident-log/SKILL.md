...
```
/incident-log [로그-경로-또는-디렉토리]
```

- 경로 없이 실행하면 CWD와 공통 로그 경로를 자동 탐색
- 특정 디렉토리 지정: `/incident-log /var/log/nginx`
- 시간 범위 지정: `/incident-log --since "2025-05-13 09:00" --until "2025-05-13 10:00"`
...
1. 인수를 파싱한다.
   - `--since`, `--until` 플래그가 있으면 시간 필터로 사용
   - 경로 인수가 있으면 해당 경로를 탐색 루트로 사용
   - 없으면 CWD + 공통 로그 경로 목록([log-patterns.md](references/log-patterns.md)의 **탐색 경로** 섹션)을 모두 탐색
2. 실제 분석 전, 발견된 파일 목록과 예상 시간 범위를 사용자에게 표시하고 진행 확인을 받는다.
